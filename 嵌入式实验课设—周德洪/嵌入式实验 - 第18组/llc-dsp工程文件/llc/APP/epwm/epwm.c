...
void EPWM1_Init(Uint16 tbprd)
{
	...
	EPwm1Regs.TBCTL.bit.PHSEN = TB_DISABLE;//相角
	...
	// 死区模块
	    ...
	    EPwm1Regs.DBRED = 45;               // 上升沿延时 DBRED*TBCLK = 200ns
	    EPwm1Regs.DBFED = 45;               // 下降沿延时 DBRED*TBCLK = 200ns
	//事件触发模块ET
	 EPwm1Regs.ETSEL.bit.SOCAEN = 1;     // 使能 ePWM2SOCA 信号产生
	 EPwm1Regs.ETSEL.bit.SOCASEL = 1;    // 当TBCTR=周期时产生 ePWM2SOCA信号
	 EPwm1Regs.ETPS.bit.SOCAPRD = 1;     // 在第1个事件时产生 ePWM2SOCA信号    // Generate INT on 1st event
...
void EPWM6_Init(Uint16 tbprd)
{
	...
	EPwm6Regs.TBCTL.bit.CLKDIV=TB_DIV1;//时钟分频1分频
	...
	// Set Compare values
	EPwm6Regs.CMPA.half.CMPA = 0;    // Set compare A value，比较值初始化
